...
export default function AnimalPage() {
    ...
    const filterAnimalsByCustomerName = async () => {
        try {
            const response = await axios.get(`http://localhost:8080/v1/animals/filterByCustomerName/${customerName}`);
    ...
    const filterAnimalsByAnimalName = async () => {
        try {
            const response = await axios.get(`http://localhost:8080/v1/animals/filterByAnimalName/${animalName}`);
            setAllAnimals(response.data.data);
        } catch (error) {
    ...
    const handleSubmit = async (event) => {
        ...
        const foundAnimal = allAnimals.find(animal => animal.name === searchTerm);
        setSearchedAnimal(foundAnimal);
    };
    ...
    const handleDelete = async (id) => {
        try {
            await axios.delete(`http://localhost:8080/v1/animals/${id}`);
    ...
    const handleSaveAnimal = async () => {
        try {
            if (updateAnimalId) {
                await axios.put(`http://localhost:8080/v1/animals/${updateAnimalId}`, newAnimalData);
                ...
                const response = await axios.post('http://localhost:8080/v1/animals', newDataWithCustomer);
