...
public class AppointmentManager implements IAppointmentService {
    ...
    public ManyResult<Appointment> getDoctorsAppointmentsByDateRange(Long doctorId, LocalDateTime start, LocalDateTime end) {
        try {
            List<Appointment> entityResult = appointmentRepo.findByDoctorIdAndAppointmentStartDateBetween(doctorId, start, end);
            if (entityResult != null) {
                ManyResult<Appointment> result = new ManyResult<>();
                result.setData(entityResult);
                result.setCode(200);
    ...
    @Override
    public ManyResult<Appointment> getAnimalsAppointmentsByDateRange(Long animalId, LocalDateTime start, LocalDateTime
            end) {
        try {
            List<Appointment> entityResult = appointmentRepo.findByAnimalIdAndAppointmentStartDateBetween(animalId, start, end);
            if (entityResult != null) {
                ManyResult<Appointment> result = new ManyResult<>();
                result.setData(entityResult);
                result.setCode(200);
